...
class DUALCNNModelv1(NNModel):
    ...
    def _create_hs_tensor_branch(algorithm_params, hs_group):
        ...
        net = DUALCNNModelv1._create_a_level(int(level_filter_count / 4), hs_group, 'level1')
        net = slim.conv2d(net, net.get_shape()[3], [1, 1], scope='connector_conv1')

        net = DUALCNNModelv1._create_a_level(int(level_filter_count / 2), net, 'level2')
        net = slim.conv2d(net, net.get_shape()[3], [1, 1], scope='connector_conv2')

        net = DUALCNNModelv1._create_a_level(int(level_filter_count), net, 'level3')
        net = slim.conv2d(net, net.get_shape()[3], [1, 1], scope='connector_conv3')

        net = DUALCNNModelv1._create_a_level(int(level_filter_count / 2), net, 'level4')
        net = slim.conv2d(net, net.get_shape()[3], [1, 1], scope='connector_conv4')

        net = DUALCNNModelv1._create_a_level(int(level_filter_count / 4), net, 'level5')
        net = slim.conv2d(net, net.get_shape()[3], [1, 1], scope='connector_conv5')

        net = DUALCNNModelv1._create_a_level(int(level_filter_count / 8), net, 'level6')
        net = slim.conv2d(net, net.get_shape()[3], [1, 1], scope='connector_conv6')

        net = DUALCNNModelv1._create_a_level(int(level_filter_count / 16), net, 'level7')
        net = slim.conv2d(net, net.get_shape()[3], [1, 1], scope='connector_conv7')

        net = DUALCNNModelv1._create_a_level(int(level_filter_count / 32), net, 'level8')
