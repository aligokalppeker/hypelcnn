...
def _shadowdata_generator_model(netinput, is_training=True):
    with slim.arg_scope(
        ...
        net6 = slim.convolution1d(net2, 1, band_size // 2, padding='SAME')
        net6 = net6 + net5

        net8 = slim.convolution1d(net6, 1, band_size, padding='SAME',
                                  normalizer_fn=None,
                                  normalizer_params=None,
                                  weights_regularizer=None,
                                  activation_fn=None)
    return tf.expand_dims(tf.expand_dims(slim.flatten(net8), axis=1), axis=1)
